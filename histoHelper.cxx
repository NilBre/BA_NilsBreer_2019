...
histoHelper::histoHelper(int pdgid) {
  ...
  NewTH1D("DeltaR_A", 100, 0., 2*M_PI, "dist(#Lambda-Ref, track) in #eta-#phi plane", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_B", 100, 0., 0.6, "dist(#Lambda-Ref, track) in #eta-#phi plane", "No. Tracks / bin", kZeroMax);
  NewTH1D("LInvMass_true_p_pi", 100, 1070., 1500.,  "M(#Lambda) for T1(p), T2(#pi)", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("LInvMass_fake_p_pi", 100, 1070., 1500., "M(#Lambda) for T1(p), T2(#pi), fake candidates", "No. of #Lambda / bin", kZeroMax);
