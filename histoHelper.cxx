...
histoHelper::histoHelper(int pdgid) {
  ...
  NewTH1D("DeltaR_A", 100, 0., 2*M_PI, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_B", 100, 0., 0.6, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("LInvMass_true_p_pi", 100, 1105., 1125.,  "M(#Lambda) for T1(p), T2(#pi)", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("LInvMass_fake_p_pi", 100, 1105., 1125., "M(#Lambda) for T1(p), T2(#pi), fake candidates", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("LInvMass_true_pi_p", 100, 1200., 3000.,  "M(#Lambda) for T1(#pi), T2(p)", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("LInvMass_fake_pi_p", 100, 1200., 3000., "M(#Lambda) for T1(#pi), T2(p), fake caandidates", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("ArithMean", 100, 1200., 2200.,  "arithm mean of both candidates", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("GeoMean", 100, 1200., 2200., "geometrical mean of both candidates", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("Min", 100, 1100., 1150., "Minimum of both masses", "No. Candidates / bin", kZeroMax);
  // NewTH1D("AntiLambdas", 50, 1107., 1127., "M(anti-#Lambda)", "No. of Anti-#Lambda / bin", kZeroMax);
  NewTH1D("PTLambda_A", 100, 0., 40., "Lambda PT [GeV]", "No. Candidates / bin", kZeroMax);
  NewTH1D("PTLambda_B", 100, 0., 40., "Lambda PT [GeV]", "No. Candidates / bin", kZeroMax);
  NewTH1D("XLambda", 100, 0., 1., "X_#Lambda", "No. / bin", kZeroMax);
  NewTH1D("DeltaR_A_fake", 100, 0., 2*M_PI, "dist(#Lambda-Ref, track) in #eta-#phi plane", "No. Tracks / bin", kZeroMax);
  NewTH1D("PTLambda_A_fake", 100, 0., 40., "Lambda PT, fakes [GeV]", "No. Candidates / bin", kZeroMax);
  NewTH1D("XLambda_fake", 100, 0., 1., "X_#Lambda fakes", "No. / bin", kZeroMax);
  NewTH1D("absvertex1", 100, 0., 100., "sqrt(vx^2 + vy^2)", "No. / bin", kZeroMax);
  NewTH1D("jetPTL", 100, 0., 100., "jet PT", "No. / bin", kZeroMax);
  NewTH1D("DeltaR_A2", 100, 0., 2*M_PI, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_A3", 100, 0., 2*M_PI, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_A2_fake", 100, 0., 2*M_PI, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_A3_fake", 100, 0., 2*M_PI, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
   // the correct order is important here, because its implicitly used in retrieving the histograms from an instance
