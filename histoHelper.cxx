...
histoHelper::histoHelper(int pdgid) {
  ...
  NewTH1D("KaonDeltaR", 50, 0., 2*M_PI, "#Delta R", "No. of Kaons / bin", kZeroMax);
  NewTH1D("JetPTforKaon", 50, 0., 90., "Jet Pt [GeV]", "No. of kaons / bin", kZeroMax);
  NewTH1D("TruthNOtherKaons", 6, 0., 6., "No. of K_{S}", "No. of kaons / bin", kZeroMax);
  NewTH1D("Charge", 5, -2., 2., "multplied charge (-1 or 1) of track_charge tuple", "No. multiplied track charges / bin", kZeroMax);
  NewTH1D("DeltaR_A", 100, 0., 1., "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
  NewTH1D("LInvMass_true_p_pi", 100, 1105., 1125.,  "#sqrt{s}(#Lambda) in [MeV]", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("LInvMass_fake_p_pi", 100, 1105., 1125., "#sqrt{s}(#Lambda) in [MeV], fake candidates", "No. of #Lambda / bin", kZeroMax);
  NewTH1D("PTLambda_A", 100, 0., 40., "pT(#Lambda) [GeV]", "No. Candidates / bin", kZeroMax);
  NewTH1D("XLambda", 100, 0., 1., "pT(#Lambda)/pT(Jet)", "No. / bin", kZeroMax);
  NewTH1D("DeltaR_A_fake", 100, 0., 2*M_PI, "dist(#Lambda-Ref, track) in #eta-#phi plane", "No. Tracks / bin", kZeroMax);
  NewTH1D("PTLambda_A_fake", 100, 0., 40., "pT(#Lambda) of fakes [GeV]", "No. Candidates / bin", kZeroMax);
  NewTH1D("XLambda_fake", 100, 0., 1., "X_{#Lambda} fakes", "No. / bin", kZeroMax);
  NewTH1D("absvertex1", 100, 0., 30., "sqrt(vx^2 + vy^2)", "No. / bin", kZeroMax);
  NewTH1D("DeltaR_A3", 100, 0., 0.5, " dR(Jet, #Lambda Kandidate)", "No. Tracks / bin", kZeroMax);
  NewTH1D("DeltaR_A3_fake", 100, 0., 0.5, "dR(#Lambda-Ref, track)", "No. Tracks / bin", kZeroMax);
   // the correct order is important here, because its implicitly used in retrieving the histograms from an instance
